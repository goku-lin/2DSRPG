...
namespace excel2json
{
    ...
    class JsonExporter
    {
        ...
        private object convertSheetToArray(DataTable sheet, bool lowcase, string excludePrefix, bool cellJson)
        {
            ...
            //TODO:在这加前置的,思路是先把下面的字典弄好
            Dictionary<string, object> segmentValue = new Dictionary<string, object>();
            for (int i = 0; i < sheet.Columns.Count; i++)
            {
                object value = sheet.Rows[0][i];
                string fieldName = sheet.Columns[i].ToString();
                if (fieldName == "end")
                    break;
            ...
            //TODO:这个得想办法，另外代码也得优化
            segmentValue["dialogsData"] = values;
            return segmentValue;
        }
        ...
        private Dictionary<string, object> convertRowToDict(DataTable sheet, DataRow row, bool lowcase, int firstDataRow, string excludePrefix, bool cellJson)
        {
            ...
            foreach (DataColumn column in sheet.Columns)
            {
                ...
                string fieldName = sheet.Rows[2][col].ToString();
                // 表头自动转换成小写
                if (lowcase)
                    fieldName = fieldName.ToLower();
